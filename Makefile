...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "spikeR", "tests")); system.time(test_check("spikeR", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran spikeR_$(VERSION).tar.gz
	rm -rf spikeR.Rcheck/
